...
async function main() {
  ...
  for (let i = 1; i <= 16; i++) {
    ...
    fs.writeFileSync('scripts/products_slow_desc.json', JSON.stringify(results, null, 2));
    // Sleep 15 seconds to be 100% safe from rate limits! (4 requests per minute limit is 15s)
    await new Promise(resolve => setTimeout(resolve, 15000));
  }
  fs.writeFileSync('scripts/products_slow_desc.json', JSON.stringify(results, null, 2));
  console.log('Finished slowly checking all products!');
}
